...
local function get_indent(ls)
    local ispc = 0
    while ls.current == ' ' or ls.current == '\t' do
        ispc = ispc + (ls.current == '\t' and 8 or 1)
        nextchar(ls)
    end
    local current_indent = ls.indent[#ls.indent]
    if ispc < current_indent then
        if ispc == ls.indent[#ls.indent - 1] then
            return -1
        else
            error("wrong indentation level")
        end
    elseif ispc == current_indent then
        return 0
    else
        ls.indent[#ls.indent + 1] = ispc
        return 1
    end
end
